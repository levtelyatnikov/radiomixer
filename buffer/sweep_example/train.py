...
def main(cfg: DictConfig):
    ...
    config = {
        'num_embeddings': cfg.dataset.num_embeddings,
        'num_iterations': cfg.model.num_iterations,
        'beta': cfg.dataset.beta,
    }
    wandb.init(project='vq_search', config=config)
    
    # Get the (possibly updated) values from wandb
    cfg.dataset.num_embeddings = wandb.config.num_embeddings
    cfg.model.num_iterations = wandb.config.num_iterations
    cfg.dataset.beta = wandb.config.beta
    ...
    if cfg.additional.debug == True:
        ...
        if cfg.additional.logger_name == 'check':
            ...
            logger = pl_loggers.WandbLogger(project=cfg.additional.logger_project_name, name=cfg.additional.logger_name)
