...
class TransitionSegmentSilenceParametersSampler(TransitionSampler):
    """Class aims to generate parameters of transition for an audio"""
    def __init__(self, configs):
        super().__init__(TransformType.TRANSITIONSEGMENTSILENCESPARAMETERSSAMPLER)
        self.configs = configs
    ...
    def _sampler(self, signal):
        ...
        Input: 
            audio_duration: sample_rate * duration_time

        Generate transition parameters for single audio.
        Output: Transition parameters dict. 
        """
        segment = signal.parameters['segment']
        segment_duration = segment[1] - segment[0]

        low_fade_in, high_fade_in  = int(self.configs['min_fade_in']*segment_duration), int(self.configs['max_fade_in']*segment_duration)
        low_fade_out, high_fade_out = int(self.configs['min_fade_out']*segment_duration), int(self.configs['max_fade_out']*segment_duration)

        transition_params = {}
        
        
        transition_params['segment_duration'] = segment_duration
        # Generate fade in number of samples
        transition_params["fade_in"] = np.random.randint(low=low_fade_in, high=high_fade_in) 
        # Generate fade out number of sampels
        transition_params["fade_out"] = np.random.randint(low=low_fade_out, high=high_fade_out) 
        transition_params['in_transition_type'] = np.random.choice(self.configs['in_transition_type'], size=1) 
        transition_params['out_transition_type'] = np.random.choice(self.configs['out_transition_type'], size=1)
