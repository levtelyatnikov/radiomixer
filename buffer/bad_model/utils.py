...
class ImageLogCallback(Callback):
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        if trainer.logger:
            with torch.no_grad():
                pl_module.eval()
                images, image_masks = pl_module.sample_images()
                trainer.logger.experiment.log({"images": [wandb.Image(images)]}, commit=False)
                trainer.logger.experiment.log({"image_masks": [wandb.Image(image_masks)]}, commit=False)
...
def normalize_audio(x: Tensor):
    x -= x.min(1, keepdim=True)[0]
    x /= x.max(1, keepdim=True)[0] + 1e-6
    return x